...
//	sql="SELECT IDcaso,tipoAbuso,tipoAsesoria,fechaInicio,fechaFin, estado,IDprofesional FROM caso where IDcaso="+id;
    ...
    public casoVo consultarCasos(casoVo id) throws SQLException {
        casoVo r = new casoVo();
//sql = "SELECT * FROM caso where IDcaso="+id;
//	sql="SELECT IDcaso,tipoAbuso,tipoAsesoria,fechaInicio,fechaFin, estado,IDprofesional FROM caso where IDcaso="+id;
        sql = "SELECT * FROM caso where IDcaso="+ r.getIDcaso();
        ...
        try {
            ...
            System.out.println("Consulta no exitosa casos " + e.getMessage());
        } finally {
            con.close();
        }
        return id;
