...
public int registrar(profesionalVo p) throws SQLException {
	...
	public List listarParaCaso() throws SQLException {
		List <profesionalVo> prof = new ArrayList<>();
		...
		try {
			...
			rs = ps.executeQuery();//ejecucion de la sentencia y guardar el resultado en el resulSet
			while(rs.next()) {
				profesionalVo a = new profesionalVo();
				a.setIDprofesional(rs.getInt(1));
